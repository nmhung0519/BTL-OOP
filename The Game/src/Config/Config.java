package Config;

public final class Config {
    public static final int NORMAL_BULLET_TTL = 30;
    public static final int NORMAL_BULLET_STRENGTH = 30;
    public static final int NORMAL_BULLET_SPEED = 3;

    public static final int MACHINE_GUN_BULLET_TTL = 15;
    public static final int MACHINE_GUN_BULLET_STRENGTH = 20;
    public static final int MACHINE_GUN_BULLET_SPEED = 4;

    public static final int SNIPER_BULLET_TTL = 60;
    public static final int SNIPER_BULLET_STRENGTH = 120;
    public static final int SNIPER_BULLET_SPEED = 5;

    public static final int ROAD_SIZE = 100;
    public static final int NORMAL_ENEMY_HEALTH = 100;
    public static final int NORMAL_ENEMY_ARMOR = 3;
    public static final int NORMAL_ENEMY_SPEED = 2;
    public static final int NORMAL_ENEMY_REWARD = 1;

    public static final int SMALLER_ENEMY_HEALTH = 50;
    public static final int SMALLER_ENEMY_ARMOR = 0;
    public static final int SMALLER_ENEMY_SPEED = 4;
    public static final int SMALLER_ENEMY_REWARD = 2;

    public static final int TANKER_ENEMY_HEALTH = 300;
    public static final int TANKER_ENEMY_ARMOR = 5;
    public static final int TANKER_ENEMY_SPEED = 1;
    public static final int TANKER_ENEMY_REWARD = 3;

    public static final int BOSS_ENEMY_HEALTH = 500;
    public static final int BOSS_ENEMY_ARMOR = 8;
    public static final int BOSS_ENEMY_SPEED = 3;
    public static final int BOSS_ENEMY_REWARD = 10;

    public static final int NORMAL_TOWER_SPEED = 30;
    public static final int NORMAL_TOWER_RANGE = 5;

    public static final int MACHINEGUN_TOWER_SPEED = 5;
    public static final int MACHINEGUN_TOWER_RANGE = 4;

    public static final int SNIPER_TOWER_SPEED = 60;
    public static final int SNIPER_TOWER_RANGE = 8;
}
